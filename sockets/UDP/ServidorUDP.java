...
public class ServidorUDP {
  public static void main(String[] args) {
    ...
    try (DatagramSocket socket = new DatagramSocket(4321)) {
      byte[] bufferRecepcion = new byte[10240];
      while (true) {
        // Recibe una peticion
        DatagramPacket datagrama = new DatagramPacket(bufferRecepcion,
            bufferRecepcion.length);
        socket.receive(datagrama);
        ByteArrayInputStream bis = new ByteArrayInputStream(bufferRecepcion);
        ...
        datagrama = new DatagramPacket(bufferEnvio, bufferEnvio.length,
            addr, port);
